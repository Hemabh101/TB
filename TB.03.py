...
#this is for extremly short term 1-day vs 2-day SMA crossovers trades
...
# =====================================
short_window = 1    # 1-day SMA
long_window = 2     # 2-day SMA

df['SMA1'] = df['Close'].rolling(window=short_window, min_periods=1).mean()
df['SMA2'] = df['Close'].rolling(window=long_window, min_periods=1).mean()

# Generate signals
df['Signal'] = (df['SMA1'] > df['SMA2']).astype(int)
...
plt.plot(df['SMA1'], label='SMA1 (1-day)', alpha=0.9)
plt.plot(df['SMA2'], label='SMA2 (2-day)', alpha=0.9)
...
plt.title('BTC Daily Trading Strategy Backtest (2023)')
plt.xlabel('Date')
plt.ylabel('Price ($)')
plt.legend()
plt.grid()
